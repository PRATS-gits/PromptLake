...
write_banner() {
    ...
    echo "    ║   Linux Edition (Read-Only Check)                            ║"
    echo "    ║                                                              ║"
    echo "    ╚══════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
test_docker() {
    write_check_item "Docker Engine"
    ...
    if command -v docker &>/dev/null; then
        ...
        if docker ps &>/dev/null; then
            ...
            write_warn "Start with: sudo systemctl start docker"
        fi
        
        # Check if user is in docker group
        if groups | grep -q docker; then
            write_pass "User is in docker group"
        else
            write_warn "User not in docker group (requires sudo for docker commands)"
            write_warn "Add with: sudo usermod -aG docker \$USER (then log out/in)"
        fi
    ...
    write_fail "Docker not found in PATH"
...
show_summary() {
    ...
    if [ $failed -eq 0 ] && [ $warnings -eq 0 ]; then
        ...
        echo -e "  ${RED}Run WebxCloud_Setup.sh to install missing tools.${NC}"
    fi
